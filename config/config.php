<?php
	...
	if (!($conn = $obj->connect()))
	{
		...
		try {
			...
			$conn->exec("CREATE TABLE `mechanic`(`fname` varchar(50) not null, `lname` varchar(50) not null, `code` int, `min_fee` double not null, `rating` double default 5, `status` varchar(50) not null default 'active', `image_path` varchar(500), `id_image_path` varchar(500), `phone` varchar(20) not null, `password` varchar(255) not null, `email` varchar(50) not null PRIMARY KEY, `lat` double default 0, `lng` double default 0) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin");
